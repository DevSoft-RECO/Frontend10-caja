  ...
  <div class="p-6 w-full space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Gestiona las peticiones enviadas y recibidas, o crea una nueva.
        </p>
      </div>
    ...
    <div class="flex border-b border-gray-200 dark:border-gray-700 select-none">
      <button
        ...
        :class="activeTrackingTab === 'recibidos' ? 'border-azul-cope text-azul-cope' : 'border-transparent text-gray-400 hover:text-gray-600 dark:hover:text-white'"
      >
      ...
      <button
        ...
        class="px-6 py-3 font-bold text-sm uppercase tracking-wider transition-all border-b-2 cursor-pointer"
        :class="activeTrackingTab === 'enviados' ? 'border-verde-cope text-verde-cope' : 'border-transparent text-gray-400 hover:text-gray-600 dark:hover:text-white'"
      >
        📤 Solicitudes de Envío de Efectivo
      </button>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div v-else-if="filteredTraslados.length === 0" class="text-center py-20 bg-gray-50 dark:bg-gray-900/40 rounded-3xl border border-dashed border-gray-250 text-gray-450 italic text-sm">
        No se encontraron peticiones en esta categoría.
      </div>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                ...
                <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">{{ activeTrackingTab === 'recibidos' ? 'Origen (Envía)' : 'Destino (Recibe)' }}</th>
              ...
              <tr v-for="t in filteredTraslados" :key="t.id" class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <td class="px-6 py-4">
                  <template v-if="activeTrackingTab === 'recibidos'">
                    <p class="font-bold text-gray-800 dark:text-gray-200">{{ t.origen_boveda?.nombre || 'Esperando Asignación de Tesorería' }}</p>
    ...
    <div v-if="showCreateModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm overflow-y-auto">
      <div class="bg-white dark:bg-gray-850 rounded-3xl w-full max-w-5xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div class="px-6 py-5 border-b border-gray-150 dark:border-gray-750 flex items-center justify-between">
          <h2 class="text-xl font-extrabold text-gray-900 dark:text-white flex items-center gap-2">
            <span class="text-azul-cope">➕</span> Nueva Petición de Efectivo
          </h2>
        ...
        <div class="p-6 overflow-y-auto flex-1 bg-gray-50/50 dark:bg-gray-900/30">
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-4 lg:col-span-1">
              <div>
                <label class="block text-xs font-bold text-gray-600 dark:text-gray-300 uppercase mb-2">Bóveda Destino (Receptora)</label>
                <select
              ...
              <div>
                <label class="block text-xs font-bold text-gray-600 dark:text-gray-300 uppercase mb-2">Comentario / Justificación</label>
                <textarea
                  v-model="comentario"
                  rows="3"
                  placeholder="Detalla la necesidad del efectivo..."
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm lg:col-span-2 flex flex-col">
              ...
              <div class="space-y-3 overflow-y-auto pr-2 max-h-[35vh] custom-scrollbar">
                <div
        ...
        <div class="px-6 py-5 border-t border-gray-150 dark:border-gray-750 bg-gray-50 dark:bg-gray-900/30 flex justify-end gap-3 rounded-b-3xl">
          ...
          <button
            @click="submitPeticion"
            :disabled="loadingSubmit || !bovedaDestinoId || totalMonto === 0"
        ...
        <div class="bg-white dark:bg-gray-850 rounded-3xl max-w-3xl w-full shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-750 flex flex-col my-auto max-h-[95vh]">
          ...
          <div class="px-6 py-5 border-b border-gray-150 dark:border-gray-750 flex items-center justify-between bg-gray-50 dark:bg-gray-900/30">
            <div class="flex items-center gap-3">
              <span class="text-lg font-extrabold text-gray-900 dark:text-white">Petición #{{ selectedTraslado.id }}</span>
              <span class="inline-flex px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-azul-cope/10 text-azul-cope">
                {{ activeTrackingTab === 'recibidos' ? 'Recibida' : 'Enviada' }}
              </span>
            </div>
          ...
          <div class="p-6 overflow-y-auto space-y-6 flex-1 custom-scrollbar">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 dark:bg-gray-900/20 p-5 rounded-2xl border border-gray-150 dark:border-gray-800">
              <div class="space-y-4">
                <div>
                  <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Origen (Despacha)</h4>
                  <p class="font-extrabold text-gray-800 dark:text-white text-base">
                    {{ selectedTraslado.origen_boveda?.nombre || 'Pendiente Tesorería' }}
                  </p>
              ...
              <div class="space-y-4">
                <div>
                  <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Monto Solicitado</h4>
            ...
            <div class="space-y-4 pt-2">
              <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider">Progreso de la Petición</h4>
            ...
            <!-- Formulario de Despacho para Enviados en estado pendiente -->
            <div v-if="activeTrackingTab === 'enviados' && selectedTraslado.estado === 'pendiente'" class="pt-6 border-t border-gray-150 dark:border-gray-750">
               <h4 class="text-sm font-extrabold text-gray-800 dark:text-white mb-4">Programar Despacho</h4>
               <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Fecha Programada</label>
                    <input type="datetime-local" v-model="formularios[selectedTraslado.id].fecha_programada" class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 text-sm font-bold" />
                  </div>
                  <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Repartidor</label>
                    <input type="text" v-model="formularios[selectedTraslado.id].repartidor" placeholder="Transporte de valores..." class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 text-sm font-bold" />
                  </div>
                  <div class="sm:col-span-2">
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-2">Comentarios Adicionales</label>
                    <input type="text" v-model="formularios[selectedTraslado.id].comentario_envio" placeholder="Opcional..." class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 text-sm font-bold" />
                  </div>
               </div>
            </div>
          ...
          <div class="px-6 py-5 border-t border-gray-150 dark:border-gray-750 flex flex-wrap items-center justify-end gap-3 bg-gray-55 dark:bg-gray-900/30 shrink-0">
            ...
            <!-- Acciones si el usuario es "Destino" (Recibidos) -->
            <template v-if="activeTrackingTab === 'recibidos'">
              <button
                v-if="selectedTraslado.estado === 'enviado'"
                @click="confirmarRecepcion(selectedTraslado.id)"
              ...
              <button
                v-if="selectedTraslado.estado === 'paquete_recibido'"
            ...
            <!-- Acciones si el usuario es "Origen" (Enviados) -->
            <template v-if="activeTrackingTab === 'enviados'">
              <button
                v-if="selectedTraslado.estado === 'pendiente'"
                @click="cancelar(selectedTraslado.id)"
              ...
              <button
                v-if="selectedTraslado.estado === 'pendiente'"
                @click="autorizarYDespachar(selectedTraslado.id)"
                :disabled="!formularios[selectedTraslado.id]?.fecha_programada || !formularios[selectedTraslado.id]?.repartidor"
                class="px-5 py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs rounded-xl shadow-md disabled:opacity-50 transition-all cursor-pointer"
              >
...
const activeTrackingTab = ref<'recibidos' | 'enviados'>('recibidos')
...
const formularios = ref<Record<number, any>>({})

const stepsPeticion = [
  { key: 'pendiente_tesoreria', states: ['pendiente_tesoreria', 'pendiente', 'enviado', 'paquete_recibido', 'ingresado'], label: 'Tesorería', icon: '📝' },
  { key: 'pendiente', states: ['pendiente', 'enviado', 'paquete_recibido', 'ingresado'], label: 'Asignado', icon: '👤' },
  { key: 'enviado', states: ['enviado', 'paquete_recibido', 'ingresado'], label: 'Despachado', icon: '🚚' },
  { key: 'paquete_recibido', states: ['paquete_recibido', 'ingresado'], label: 'Recibido', icon: '📦' },
...
const filteredTraslados = computed(() => {
  return trasladosList.value.filter(t => {
    if (activeTrackingTab.value === 'recibidos') {
...
const formatEstado = (estado: string) => {
  const labels: Record<string, string> = {
    pendiente_tesoreria: 'En Tesorería',
    pendiente: 'Asignado',
    enviado: 'Enviado',
...
const fetchTraslados = async () => {
  ...
  try {
    const res = await axios.get('/cajas/peticiones-efectivo/propias', { params: { caja_id: bovedaDestinoId.value } })
    trasladosList.value = res.data
    res.data.forEach((t: any) => {
      if (!(t.id in formularios.value)) {
        formularios.value[t.id] = { fecha_programada: '', repartidor: '', comentario_envio: '' }
      }
    })
  } catch (error) {} finally { loadingList.value = false }
}

const submitPeticion = async () => {
  ...
  try {
    await axios.post('/cajas/peticiones-efectivo', {
      boveda_destino_id: bovedaDestinoId.value,
      comentario_peticion: comentario.value,
      detalles
    })
    successMsg.value = 'Petición enviada a Tesorería exitosamente.'
    comentario.value = ''
    denominacionesList.value.forEach(d => d.cantidad_enviar = undefined)
    showCreateModal.value = false
    activeTrackingTab.value = 'recibidos'
    fetchTraslados()
    setTimeout(() => successMsg.value = '', 5000)
  } catch (error: any) {
    errorMsg.value = error.response?.data?.message || 'Error al enviar petición'
    setTimeout(() => errorMsg.value = '', 5000)
  } finally { loadingSubmit.value = false }
}

const autorizarYDespachar = async (id: number) => {
  const form = formularios.value[id]
  if (!form.fecha_programada || !form.repartidor) return
  errorMsg.value = ''; successMsg.value = ''
  try {
    await axios.post(`/cajas/peticiones-efectivo/${id}/autorizar-despachar`, form)
    successMsg.value = `Petición #${id} despachada y egreso registrado exitosamente.`
    selectedTraslado.value = null
    fetchTraslados()
    setTimeout(() => successMsg.value = '', 5000)
  } catch (error: any) {
    errorMsg.value = error.response?.data?.message || 'Error al autorizar y despachar'
    setTimeout(() => errorMsg.value = '', 5000)
  }
}

const cancelar = async (id: number) => {
  if (!confirm('¿Seguro que deseas cancelar esta petición?')) return
  try {
    await axios.delete(`/cajas/peticiones-efectivo/${id}`)
    successMsg.value = `Petición #${id} cancelada.`
    selectedTraslado.value = null
    fetchTraslados()
    setTimeout(() => successMsg.value = '', 5000)
  } catch (error: any) {
...
const confirmarRecepcion = async (id: number) => {
  try {
    await axios.post(`/cajas/peticiones-efectivo/${id}/confirmar-recepcion`)
    successMsg.value = 'Paquete recibido correctamente.'
    selectedTraslado.value = null
    fetchTraslados()
    setTimeout(() => successMsg.value = '', 5000)
  } catch (error) {}
...
const ingresarEfectivo = async (id: number) => {
  try {
    await axios.post(`/cajas/peticiones-efectivo/${id}/ingresar-efectivo`)
    successMsg.value = 'Efectivo ingresado a bóveda correctamente.'
    selectedTraslado.value = null
    fetchTraslados()
    setTimeout(() => successMsg.value = '', 5000)
  } catch (error) {}
